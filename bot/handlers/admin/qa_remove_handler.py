...
async def remove_question_answer(message: Message, state: FSMContext):
    if await user_service.has_user_record(message.from_user.id):
        ...
        if has_required_role:
            ...
            await QARemoveFSM.waiting_for_id.set()
        else:
...
async def wait_for_id(message: Message, state: FSMContext):
    if str.isdigit(message.text):
        if await qa_service.has_qa(int(message.text)):
            await qa_service.remove_qa(int(message.text))
...
def register_qa_remove_handler(dp: Dispatcher):
    dp.register_message_handler(remove_question_answer, commands=["qa_remove"], state=None)
    dp.register_message_handler(wait_for_id, state=QARemoveFSM.waiting_for_id)
