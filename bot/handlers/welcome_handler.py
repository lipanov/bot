...
async def wait_for_language(message: Message, state: FSMContext):
    if message.text in LANGUAGES:
        ...
        await bot.send_message(message.from_user.id, welcome_message(language_key), reply_markup=create_remove_kb())
...
async def wait_for_name(message: Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, who_are_you_message(language_key, name), reply_markup=create_list_kb(who_list))
...
async def wait_for_who(message: Message, state: FSMContext):
    ...
    if message.text in who_list:
        ...
        if who == student_who:
            await bot.send_message(message.from_user.id, edu_from_you_studying_message(language_key), reply_markup=create_list_kb(edu_forms))
        elif who == enrollee_who:
            await bot.send_message(message.from_user.id, edu_from_you_plan_message(language_key), reply_markup=create_list_kb(edu_forms))
